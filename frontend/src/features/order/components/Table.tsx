import type { Item, OrderCustomer } from "../intarfaces/Order";
        ...
        <table className="table-fixed">
          <thead className="bg-gray-100">
            <tr className="text-center">
              <th className={`min-w-64 ${thClass}`}>Customer Name</th>
              <th className={`min-w-32 ${thClass}`}>Order Date</th>
              <th className={`min-w-32 ${thClass}`}>Order Price</th>
          ...
          <tbody>
            {orders.length > 0 ? (
              orders.map((order: OrderCustomer) => (
                <tr key={order.id}>
                  <td className={tdClass}>{order.customer.name}</td>
